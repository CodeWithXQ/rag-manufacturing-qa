...
| 指标 | 纯向量 | 纯 BM25 | 混合检索 |
|---|---|---|---|
| Recall@1 | 0.00% | 4.00% | **20.00%** |
| Recall@3 | 54.00% | 30.00% | **58.00%** |
| Recall@5 | **72.00%** | 44.00% | 68.00% |
| **MRR** | 0.3010 | 0.1777 | **0.3870** |

**结论解读（消融贡献拆解）**：

- **向量检索 = 召回强、排序弱**：Recall@5 高达 72%（能召回），但 Recall@1 为 0%（top1 总是"标题/现象"块，而非真正的"排查/原因"段落）。它懂语义，但不够精确。
- **BM25 = 召回弱、排序也弱**：Recall@5 只有 44%（纯词面匹配，漏掉同义词/语义相近的段落），MRR 仅 0.1777（分数只看词频，排序不准）。它的价值在于"精确词面命中"（Recall@1 有 4%），是向量检索没有的补充信号。
- **混合检索 = 取长补短**：向量负责"召回全"，BM25 补"精确命中"，RRF + rerank 负责"排序准"。结果 Recall@1 从 0%→20%（+20%）、MRR 从 0.301→0.387（+28.6%）。
- **诚实边界**：Recall@5 略降（72%→68%），说明混合检索的价值在"把对的排更靠前"（排序质量），不在"召回更多"（召回本来就够）。这是真实现象，面试能讲清。

### Bad case 分析（16 题未命中，根因高度一致）
...
```bash
python evaluate_generation.py    # 全量 50 题（生成 + judge）
```

检索只回答"找到了没有"，生成质量才回答"最终答案准不准"。链路：问题 → 混合检索 top-5 → LLM 生成（答案带 [i] 引用）→ 评测。

| 指标 | 数值 | 口径 |
|---|---|---|
| 引用覆盖率 | 100% | 50 题答案全部标注了 [i] 引用编号 |
| 引用命中率 | 98% | 49/50 题答案引用的文档命中标准答案文档 gold_doc |
| 幻觉率 | 0%（人工复核） | LLM-as-judge 初筛 6%，人工抽查 3 例全为误判 |

**引用命中率唯一 miss（题 42「气动系统气源压力不足」）**：答案内容其实覆盖了正确排查方向，但引用编号张冠李戴——内容该指向 42 文档，却标到了其他文档的 [4][5]。属于"内容对、引用错"，是引用解析层可修的工程问题。

**幻觉率为什么要人工复核**：LLM-as-judge 初筛判 3 题"幻觉"（题 1/28/40），人工核对全是误判——这 3 题的答案反而质量最高，都主动写了"参考资料未提供…建议进一步检查"或"跨设备类比"这类**诚实边界声明**，被 judge 当成幻觉。结论：这 50 题无编造型幻觉；同时暴露 LLM-as-judge 需人工抽样校准、不能单独当金标准（也是当前 RAG 评测的公开痛点）。
